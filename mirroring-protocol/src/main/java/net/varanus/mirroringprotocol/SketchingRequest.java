...
public final class SketchingRequest
{
    private final DirectedNodePort switchPort;
    private final Flow             flow;
    private final Duration         collDuration;

    public SketchingRequest( DirectedNodePort switchPort,
                             Flow flow,
    ...
    {
        private static final long REQUEST_PREAMBLE = 0x18e9fe4b7f3eb1d2L;

        public static IOWriter<SketchingRequest> writer( CollectorId collectorId, Logger log )
        {
            MoreObjects.requireNonNull(collectorId, "collectorId", log, "log");
            return new IOWriter<SketchingRequest>() {
                @Override
                public void write( SketchingRequest request, WritableByteChannel ch ) throws IOChannelWriteException
                {
                    ...
                    if (log.isTraceEnabled()) {
                        log.trace("Writing sketching request to collector {}: request preamble 0x{}",
                            collectorId, Long.toHexString(REQUEST_PREAMBLE));
                    }
                    Serializers.longWriter(ByteOrder.BIG_ENDIAN).writeLong(REQUEST_PREAMBLE, ch);

                    log.trace("Writing sketching request to collector {}: switch-port {}", collectorId, switchPort);
                    DirectedNodePort.IO.writer().write(switchPort, ch);

                    log.trace("Writing sketching request to collector {}: flow {}", collectorId, flow);
                    Flow.IO.writer().write(flow, ch);

                    if (log.isTraceEnabled()) {
                        log.trace("Writing sketching request to collector {}: collect duration of {}",
        ...
        public static IOReader<SketchingRequest> reader( CollectorId collectorId,
                                                         Function<DatapathId, String> idAliaser,
                                                         Logger log )
        {
            MoreObjects.requireNonNull(collectorId, "collectorId", idAliaser, "idAliaser", log, "log");
            return new IOReader<SketchingRequest>() {
                @Override
                public SketchingRequest read( ReadableByteChannel ch ) throws IOChannelReadException
                {
                    ...
                    if (log.isTraceEnabled()) {
                        log.trace("Read sketching request to collector {}: request preamble 0x{}",
                            collectorId, Long.toHexString(preamble));
                    }

                    final DirectedNodePort switchPort = DirectedNodePort.IO.reader(idAliaser).read(ch);
                    log.trace("Read sketching request to collector {}: switch-port {}", collectorId, switchPort);

                    final Flow flow = Flow.IO.reader().read(ch);
                    log.trace("Read sketching request to collector {}: flow {}", collectorId, flow);

                    final Duration colDuration = Serializers.durationReader().read(ch);
                    if (log.isTraceEnabled()) {
                        log.trace("Read sketching request to collector {}: collect duration of {}",
                            collectorId, TimeUtils.toSmartDurationString(colDuration));
                    }

                    return new SketchingRequest(switchPort, flow, colDuration);
                }
            };
        }
